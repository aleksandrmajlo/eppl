...
class ControllerExtensionFeedUnixml extends Controller {
  ...
  private $error = array();
  private $ext = 'unixml';
  private $path = 'extension/feed/unixml';
  ...
  public function index() {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
      ...
      $this->response->redirect($this->url->link($this->path, 'user_token=' . $data['token'] . '&type=feed', true));
    ...
    foreach(array($this->language->get('text_home') => 'common/dashboard',$this->language->get('text_extension') => $this->module_path,$data['heading_title_module'] => $this->path) as $breadcrumb_text => $breadcrumb_link){
      $data['breadcrumbs'][] = array(
        ...
        'href' => $this->url->link($breadcrumb_link, 'user_token=' . $this->session->data['user_token'] . '&type=feed', true)
      );
    }

    $data['action'] = $this->url->link($this->path, 'user_token=' . $this->session->data['user_token'], true);
    $data['cancel'] = $this->url->link($this->module_path, 'user_token=' . $this->session->data['user_token'] . '&type=feed', true);
      ...
      foreach($fields as $func => $field){
        ...
        if($field == 'categories' or $field == 'manufacturers' or $field == 'option_multiplier_id'){
          ...
          if (isset($this->request->post[$this->varname($field, $feed)])) {
            ...
            $data[$this->varname($field, $feed)] = 10000;
          }
          //3.x
    ...
    $data_info = @file_get_contents('https://microdata.pro/index.php?route=sale/' . $this->ext . 'info');
    if($data_info){
    ...
    $data['export_setting'] = $this->url->link($this->path . '/export_setting', 'user_token=' . $data['token'], true);
    $data['import_setting'] = $this->url->link($this->path . '/import_setting', 'user_token=' . $data['token'], true);
